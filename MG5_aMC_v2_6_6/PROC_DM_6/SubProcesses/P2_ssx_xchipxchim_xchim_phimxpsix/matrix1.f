...
C     
C     Process: s s~ > ~chi+ ~chi- WEIGHTED<=4 @2
C     *   Decay: ~chi- > phi- ~psi~ WEIGHTED<=2
C     
      USE DISCRETESAMPLER
      IMPLICIT NONE
C     
...
C     
      ...
      DATA (NHEL(I,   1),I=1,5) / 1,-1,-1, 0, 1/
      DATA (NHEL(I,   2),I=1,5) / 1,-1,-1, 0,-1/
      DATA (NHEL(I,   3),I=1,5) / 1,-1, 1, 0, 1/
      DATA (NHEL(I,   4),I=1,5) / 1,-1, 1, 0,-1/
      DATA (NHEL(I,   5),I=1,5) / 1, 1,-1, 0, 1/
      DATA (NHEL(I,   6),I=1,5) / 1, 1,-1, 0,-1/
      DATA (NHEL(I,   7),I=1,5) / 1, 1, 1, 0, 1/
      DATA (NHEL(I,   8),I=1,5) / 1, 1, 1, 0,-1/
      DATA (NHEL(I,   9),I=1,5) /-1,-1,-1, 0, 1/
      DATA (NHEL(I,  10),I=1,5) /-1,-1,-1, 0,-1/
      DATA (NHEL(I,  11),I=1,5) /-1,-1, 1, 0, 1/
      DATA (NHEL(I,  12),I=1,5) /-1,-1, 1, 0,-1/
      DATA (NHEL(I,  13),I=1,5) /-1, 1,-1, 0, 1/
      DATA (NHEL(I,  14),I=1,5) /-1, 1,-1, 0,-1/
...
C     Needed for v4 models
      ...
      DOUBLE PRECISION FK_MDL_WPHI
      DOUBLE PRECISION FK_MDL_WPSI
      DOUBLE PRECISION FK_MDL_WCHI
      SAVE FK_MDL_WZ
      SAVE FK_ZERO
      SAVE FK_MDL_WPHI
      SAVE FK_MDL_WPSI
...
C     ----------
      IF (FIRST) THEN
        FIRST=.FALSE.
        FK_MDL_WPHI = SIGN(MAX(ABS(MDL_WPHI), ABS(MDL_MPHI
     $   *SMALL_WIDTH_TREATMENT)), MDL_WPHI)
        FK_MDL_WCHI = SIGN(MAX(ABS(MDL_WCHI), ABS(MDL_MCHI
     $   *SMALL_WIDTH_TREATMENT)), MDL_WCHI)
     ...
     $   , ZERO)
        FK_MDL_WPSI = SIGN(MAX(ABS(MDL_WPSI), ABS(MDL_MPSI
     $   *SMALL_WIDTH_TREATMENT)), MDL_WPSI)
      ENDIF
      ...
      CALL IXXXXX(P(0,5),MDL_MPSI,NHEL(5),-1*IC(5),W(1,5))
      CALL FFS1_2(W(1,5),W(1,4),GC_124,MDL_MCHI, FK_MDL_WCHI,W(1,6))
      CALL FFV1P0_3(W(1,1),W(1,2),GC_1,ZERO, FK_ZERO,W(1,5))
C     Amplitude(s) for diagram number 1
      CALL FFV1_0(W(1,6),W(1,3),W(1,5),GC_4,AMP(1))
      CALL FFV2_3_3(W(1,1),W(1,2),GC_83,GC_76,MDL_MZ, FK_MDL_WZ,W(1,5))
C     Amplitude(s) for diagram number 2
      CALL FFV1_0(W(1,6),W(1,3),W(1,5),GC_78,AMP(2))
