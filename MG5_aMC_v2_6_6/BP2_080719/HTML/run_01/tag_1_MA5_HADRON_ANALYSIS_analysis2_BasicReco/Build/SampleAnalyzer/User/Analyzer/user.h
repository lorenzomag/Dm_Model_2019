...
class user : public AnalyzerBase
{
 ...
 private : 
  ...
  bool isP__bPTorderingfinalstate(const MCParticleFormat* part) const {
  ...
  bool isP__ePTorderingfinalstate(const MCParticleFormat* part) const {
     if ( part==0 ) return false;
     if ( !PHYSICS->Id->IsFinalState(part) ) return false;
     if ( (part->pdgid()!=-11)&&(part->pdgid()!=11) ) return false;
     return true; }
  std::vector<const MCParticleFormat*> _P_jPTorderingfinalstate_REG_;
  bool isP__jPTorderingfinalstate(const MCParticleFormat* part) const {
     if ( part==0 ) return false;
     if ( !PHYSICS->Id->IsFinalState(part) ) return false;
  ...
  bool isP__muPTorderingfinalstate(const MCParticleFormat* part) const {
     if ( part==0 ) return false;
     if ( !PHYSICS->Id->IsFinalState(part) ) return false;
