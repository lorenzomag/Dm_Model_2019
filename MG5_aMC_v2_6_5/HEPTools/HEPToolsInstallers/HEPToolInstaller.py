...
_HepTools = {'hepmc':
             ...
             'boost':
               {'install_mode':'Default',
                'version':       '1.59.0',
                'www': 'http://sourceforge.net/projects/boost/files/boost/1.59.0', 
                'tarball':      ['online','%(www)s/boost_1_59_0.tar.gz'],
                'mandatory_dependencies': [],
                'optional_dependencies' : [],
             ...
             'gsl':{'install_mode':'Default',
                'version':       '2.4',
                'www': 'http://mirror.ibcp.fr/pub/gnu/gsl',
             ...
             'pythia8':
               {'install_mode':'Default',
                'version':       '82151',
                'www': 'http://home.thep.lu.se/~torbjorn/pythia8',
# Official version
                'tarball':      ['online','%(www)s/pythia82.tgz'],
# Development version
#                'tarball':      ['online','http://slac.stanford.edu/~prestel/pythia8217alpha.tar.gz'],                
             ...
             'lhapdf6':
               {'install_mode':'Default',
                'version':       '6.1.6',
                'www': 'http://www.hepforge.org/archive/lhapdf',
...
#                'tarball':      ['online','http://madgraph.phys.ucl.ac.be/Downloads/MG5aMC_PY8_interface.tar.gz'],
               ...
               {'install_mode':'Default',
                ...
                'www' : 'https://bitbucket.org/peraro/ninja/downloads',
                'tarball':      ['online','%(www)s/ninja-latest.tar.gz'],
               ...
               {'install_mode':'Default',
                ...
                'tarball':      ['online','%(www)s/ma5_latest.tgz'],
             ...
             'ginac':
                 {'install_mode': 'Default',
                  'version': '1.7.5',
                  'www' : 'https://ginac.de',
                  'tarball': ['online', '%(www)s/ginac-1.7.5.tar.bz2'],
             ...
             'cln':
                 {'install_mode': 'Default',
                  ...
                  'tarball': ['online', '%(www)s/cln-1.3.4.tar.bz2'],
                  'mandatory_dependencies': [],
                  'optional_dependencies': [],
                  'libraries': [''],
...
available_options = ['--prefix','--fortran_compiler','--cpp_compiler','--gfortran_compiler','--update',
                    ...
                    ['--with_%s'%tool for tool in _HepTools.keys()+_non_installable_dependencies.keys()+
                                                                           _dependency_names_map.keys()]+\
                    ['--veto_%s'%tool for tool in _HepTools.keys()+_non_installable_dependencies.keys()]+\
    ...
    for path in [m.name for m in tarfile.open(_HepTools['collier']['tarball'][1],'r:gz').getmembers()]:
        ...
        if not match is None:
            ...
            if _HepTools['collier']['version'] != collier_version:
                logger.info("HEPToolsInstaller is correction COLLIER version to be %s"%collier_version)
...
def install_madanalysis5(tmp_path):
    ...
    for dep in ['delphes','delphesMA5tune','root','fastjet','pad','padforMA5tune']:
...
def install_pythia8(tmp_path):
    ...
    try:
        ...
        try:
            ...
            pass
        pass
    cxx_common = ['-ldl','-fPIC',_cpp_standard_lib]
