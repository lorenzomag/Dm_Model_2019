...
C     
C     Process: e+ ve > w+ > e+ ve a WEIGHTED<=6 @1
C     Process: e+ ve > w+ > mu+ vm a WEIGHTED<=6 @1
C     Process: mu+ vm > w+ > e+ ve a WEIGHTED<=6 @1
C     Process: mu+ vm > w+ > mu+ vm a WEIGHTED<=6 @1
C     
...
C     
      ...
      DOUBLE PRECISION MUP1,EP1
      DOUBLE PRECISION VE2,VM2
...
C     Continue only if IMODE is 0, 4 or 5
      ...
      IF (ABS(LPP(IB(1))).GE.1) THEN
        LP=SIGN(1,LPP(IB(1)))
        MUP1=PDG2PDF(ABS(LPP(IB(1))),-13*LP, 1,XBK(IB(1))
     $   ,DSQRT(Q2FACT(1)))
     ...
     $   ,DSQRT(Q2FACT(1)))
      ...
      IF (ABS(LPP(IB(2))).GE.1) THEN
        LP=SIGN(1,LPP(IB(2)))
        VE2=PDG2PDF(ABS(LPP(IB(2))),12*LP, 1,XBK(IB(2)),DSQRT(Q2FACT(2)
     $   ))
        VM2=PDG2PDF(ABS(LPP(IB(2))),14*LP, 1,XBK(IB(2)),DSQRT(Q2FACT(2)
     $   ))
      ENDIF
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! e+ ve > e+ ve a
      PD(IPROC)=EP1*VE2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! e+ ve > mu+ vm a
      PD(IPROC)=EP1*VE2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! mu+ vm > e+ ve a
      PD(IPROC)=MUP1*VM2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! mu+ vm > mu+ vm a
      PD(IPROC)=MUP1*VM2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
